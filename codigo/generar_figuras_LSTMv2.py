...
class FluidoLSTM(nn.Module):
    def __init__(self, hidden_size, num_lstmlayers=1, output_size=25):
        super(FluidoLSTM, self).__init__()
        # input_size=1: cada timestep es un único escalar de velocidad
    ...
    def forward(self, x):
        # x: (batch, 250) → (batch, 250, 1): 250 pasos temporales, 1 feature cada uno
        x = x.unsqueeze(-1)
        lstm_out, _ = self.lstm(x)                    # (batch, 250, hidden_size)
        prediccion = self.linear(lstm_out[:, -1, :])  # solo el último estado oculto
        return prediccion         
...
modelo_inf = FluidoLSTM(hidden_size=lstm_hidden_size,
