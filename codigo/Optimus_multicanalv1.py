    ...
    if os.path.exists(train_dataset_pt_tf) and os.path.exists(val_dataset_pt_tf) and os.path.exists(stats_pt_tf):
        ...
        def crear_ventanas_cfd(matriz, lookback, predict, stride):
            ...
            for i in range(0, num_tiempos - lookback - predict + 1, stride):
                # 🚨 ENTRADA (X): Líneas 0, 1, 2, 3, 4 (5 Canales)
                bloque_x = matriz[i : i + lookback, 0:5, :]       
                
                # 🚨 SALIDA (Y): Solo la línea 5 (El canal objetivo de entrenamiento)
                bloque_y = matriz[i + lookback : i + lookback + predict, 5:6, :] 
        ...
        X_train_np_tf, y_train_np_tf = crear_ventanas_cfd(matriz_train, seq_x, seq_y, salto)
        X_val_np_tf, y_val_np_tf = crear_ventanas_cfd(matriz_val, seq_x, seq_y, salto)
...
f_train = 'train_dataset_tfmulticanal1.pt'
f_val = 'val_dataset_tfmulticanal1.pt'
f_stats = 'norm_stats_tfmulticanal1.pt'
